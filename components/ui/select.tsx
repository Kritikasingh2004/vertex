...
export function Select({ label = "Select", className, options = ["Most Relevant", "Newest", "Popular"] }: SelectProps) {
  return (
    <label className={cn("relative block", className)}>
      ...
      <ChevronDown aria-hidden="true" className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-neutral-500" size={16} />
    </label>
  );
}